...
class ScNode:
    ...
    def update_value(self, context):
        if (
            hasattr(context.space_data, "edit_tree")
            and context.space_data.edit_tree.bl_idname == "ScNodeTree"
        ):
    ...
    def draw_buttons(self, context, layout):
        if self.node_executable:
            if self == context.space_data.edit_tree.nodes.active:
                if not self == context.space_data.edit_tree.nodes.get(
                    str(context.space_data.edit_tree.node)
                ):
