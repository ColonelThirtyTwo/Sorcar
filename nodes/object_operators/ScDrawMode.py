...
class ScDrawMode(Node, ScObjectOperatorNode):
    ...
    in_display_type: EnumProperty(
        items=[("SOLID", "Solid", ""), ("WIRE", "Wire", ""), ("BOUNDS", "Bounds", "")],
        default="SOLID",
        update=ScNode.update_value,
    )
    in_bounds: BoolProperty(update=ScNode.update_value)
    in_bounds_shape: EnumProperty(
        items=[
            ("BOX", "Box", ""),
            ("SPHERE", "Sphere", ""),
            ("CYLINDER", "Cylinder", ""),
    ...
    def init(self, context):
        ...
        self.inputs.new("ScNodeSocketString", "Display As").init(
            "in_display_type", True
        )
    ...
    def error_condition(self):
        return (
            ...
            or (
                not self.inputs["Display As"].default_value
                in ["SOLID", "WIRE", "BOUNDS"]
            )
            or (
                not self.inputs["Bounds Shape"].default_value
                in ["BOX", "SPHERE", "CYLINDER", "CONE", "CAPSULE"]
            )
        )

    def functionality(self):
        self.inputs["Object"].default_value.show_name = self.inputs[
            "Name"
        ].default_value
        self.inputs["Object"].default_value.show_axis = self.inputs[
            "Axis"
        ].default_value
        self.inputs["Object"].default_value.show_wire = self.inputs[
            "Wireframe"
        ].default_value
        self.inputs["Object"].default_value.show_all_edges = self.inputs[
            "All Edges"
        ].default_value
        self.inputs["Object"].default_value.show_in_front = self.inputs[
            "In Front"
        ].default_value
        self.inputs["Object"].default_value.display.show_shadows = self.inputs[
            "Shadow"
        ].default_value
        self.inputs["Object"].default_value.display_type = self.inputs[
            "Display As"
        ].default_value
        self.inputs["Object"].default_value.show_bounds = self.inputs[
            "Bounds"
        ].default_value
        self.inputs["Object"].default_value.display_bounds_type = self.inputs[
            "Bounds Shape"
        ].default_value
        self.inputs["Object"].default_value.hide_viewport = not self.inputs[
            "Viewport"
        ].default_value
        self.inputs["Object"].default_value.hide_render = not self.inputs[
            "Render"
        ].default_value
