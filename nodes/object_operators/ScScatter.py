...
class ScScatter(Node, ScObjectOperatorNode):
    ...
    prop_loc: EnumProperty(
        items=[("X", "X", "", 2), ("Y", "Y", "", 4), ("Z", "Z", "", 8)],
        default={"X", "Y", "Z"},
        options={"ENUM_FLAG"},
        update=ScNode.update_value,
    )
    prop_rot: EnumProperty(
        items=[("X", "X", "", 2), ("Y", "Y", "", 4), ("Z", "Z", "", 8)],
        default={"X", "Y", "Z"},
        options={"ENUM_FLAG"},
        update=ScNode.update_value,
    ...
    def error_condition(self):
        return (
            ...
            or (
                not self.inputs["Component"].default_value
                in ["FACES", "VERTS", "EDGES"]
            )
            or (not self.inputs["Type"].default_value in ["INST", "MAN", "RAND"])
            or (
                self.inputs["Type"].default_value == "INST"
                and self.inputs["Component"].default_value == "EDGES"
            )
        )
    ...
    def functionality(self):
        if self.inputs["Type"].default_value == "INST":
            ...
            if self.inputs["Component"].default_value == "FACES":
                ...
                loc = [
                    (
                        Vector(
                            self.inputs["Object"]
                            .default_value.data.vertices[i.vertices[0]]
                            .co
                        )
                        + Vector(
                            self.inputs["Object"]
                            .default_value.data.vertices[i.vertices[1]]
                            .co
                        )
                    )
                    / 2
            ...
            for i in range(0, len(loc)):
                ...
                rot_diff = (
                    Vector((0.0, 0.0, 1.0)).rotation_difference(rot[i]).to_euler()
                )
