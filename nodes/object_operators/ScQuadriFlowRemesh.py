...
class ScQuadriFlowRemesh(Node, ScObjectOperatorNode):
    ...
    in_target_edge_length: FloatProperty(
        default=0.1, min=0.0000001, update=ScNode.update_value
    )
    ...
    def init(self, context):
        ...
        self.inputs.new("ScNodeSocketNumber", "Edge Length").init(
            "in_target_edge_length"
        )
        self.inputs.new("ScNodeSocketNumber", "Number of Faces").init("in_target_faces")
        self.inputs.new("ScNodeSocketNumber", "Old Object Face Area").init(
            "in_mesh_area"
        )
        self.inputs.new("ScNodeSocketNumber", "Seed").init("in_seed", True)
        self.inputs.new("ScNodeSocketBool", "Use Paint Symmetry").init(
            "in_paint_symmetry"
        )
        self.inputs.new("ScNodeSocketBool", "Preserve Sharp").init("in_preserve_sharp")
        self.inputs.new("ScNodeSocketBool", "Preserve Mesh Boundary").init(
            "in_preserve_boundary"
        )
        self.inputs.new("ScNodeSocketBool", "Use Mesh Curvature").init(
            "in_mesh_curvature"
        )
        self.inputs.new("ScNodeSocketBool", "Preserve Paint Mask").init(
            "in_preserve_paint_mask"
        )
        self.inputs.new("ScNodeSocketBool", "Smooth Normals").init(
            "in_smooth_normals", True
        )
