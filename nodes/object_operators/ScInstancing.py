...
class ScInstancing(Node, ScObjectOperatorNode):
    ...
    in_type: EnumProperty(
        items=[("NONE", "None", ""), ("VERTS", "Vertices", ""), ("FACES", "Faces", "")],
        default="NONE",
    ...
    in_factor: FloatProperty(
        default=1.0, min=0.001, max=10000, update=ScNode.update_value
    )
    ...
    def init(self, context):
        ...
        self.inputs.new("ScNodeSocketBool", "Display Instancer").init(
            "in_display", True
        )
    ...
    def error_condition(self):
        return (
            ...
            or (
                self.inputs["Factor"].default_value < 0.001
                or self.inputs["Factor"].default_value > 10000
            )
        )

    def functionality(self):
        self.inputs["Object"].default_value.instance_type = self.inputs[
            "Type"
        ].default_value
        self.inputs["Object"].default_value.show_instancer_for_viewport = self.inputs[
            "Display Instancer"
        ].default_value
        self.inputs["Object"].default_value.show_instancer_for_render = self.inputs[
            "Render Instancer"
        ].default_value
        self.inputs["Object"].default_value.use_instance_faces_scale = self.inputs[
            "Scale by Face Size"
        ].default_value
        self.inputs["Object"].default_value.instance_faces_scale = self.inputs[
            "Factor"
        ].default_value
        self.inputs[
            "Object"
        ].default_value.use_instance_vertices_rotation = self.inputs[
            "Rotate by Vertex Normal"
        ].default_value
