...
class ScPivotPoint(Node, ScSettingNode):
    ...
    in_pivot: EnumProperty(
        items=[
            ("BOUNDING_BOX_CENTER", "Bound Box Center", ""),
            ("CURSOR", "Cursor", ""),
    ...
    def error_condition(self):
        return super().error_condition() or (
            not self.inputs["Pivot Point"].default_value
            in [
                "BOUNDING_BOX_CENTER",
                "CURSOR",
                "INDIVIDUAL_ORIGINS",
    ...
    def functionality(self):
        bpy.context.scene.tool_settings.transform_pivot_point = self.inputs[
            "Pivot Point"
        ].default_value
        bpy.context.scene.tool_settings.use_transform_pivot_point_align = self.inputs[
            "Only Origins"
        ].default_value
