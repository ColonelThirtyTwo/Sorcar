...
class ScSnap(Node, ScSettingNode):
    ...
    prop_elements: EnumProperty(
        ...
        items=[
            ...
            ("FACE", "Face", "", 8),
            ("VOLUME", "Volume", "", 16),
            ("EDGE_MIDPOINT", "Edge Center", "", 32),
            ("EDGE_PERPENDICULAR", "Edge Perpendicular", "", 64),
        ],
        default={"INCREMENT"},
        options={"ENUM_FLAG"},
        update=ScNode.update_value,
    )
    prop_affect: EnumProperty(
        name="Affect",
        items=[
            ("MOVE", "Move", "", 1),
            ("ROTATE", "Rotate", "", 2),
    ...
    def init(self, context):
        ...
        self.inputs.new("ScNodeSocketBool", "Project Individual Elements").init(
            "in_project"
        )
    ...
    def error_condition(self):
        return (
            ...
            or (
                not self.inputs["Snap With"].default_value
                in ["CLOSEST", "CENTER", "MEDIAN", "ACTIVE"]
            )
        )
    ...
    def functionality(self):
        ...
        bpy.context.scene.tool_settings.snap_target = self.inputs[
            "Snap With"
        ].default_value
        bpy.context.scene.tool_settings.use_snap_grid_absolute = self.inputs[
            "Absolute Grid Snap"
        ].default_value
        bpy.context.scene.tool_settings.use_snap_backface_culling = self.inputs[
            "Backface Culling"
        ].default_value
        bpy.context.scene.tool_settings.use_snap_peel_object = self.inputs[
            "Snap Peel Object"
        ].default_value
        bpy.context.scene.tool_settings.use_snap_project = self.inputs[
        ...
        bpy.context.scene.tool_settings.use_snap_self = self.inputs[
            "Project onto Self"
        ].default_value
        bpy.context.scene.tool_settings.use_snap_align_rotation = self.inputs[
