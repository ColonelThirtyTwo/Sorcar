...
class ScSetSelectionMode(Node, ScNode):
    ...
    in_selection_type: EnumProperty(
        name="Mode",
        items=[
            ("VERT", "Vertices", "", "VERTEXSEL", 1),
            ("EDGE", "Edges", "", "EDGESEL", 2),
            ("FACE", "Faces", "", "FACESEL", 4),
        ],
        default={"VERT"},
        options={"ENUM_FLAG"},
    ...
    def init(self, context):
        ...
        self.inputs.new("ScNodeSocketSelectionType", "Selection Type").init(
            "in_selection_type", True
        )
    ...
    def functionality(self):
        bpy.context.tool_settings.mesh_select_mode = [
            "VERT" in self.inputs["Selection Type"].default_value,
            "EDGE" in self.inputs["Selection Type"].default_value,
            "FACE" in self.inputs["Selection Type"].default_value,
        ]
