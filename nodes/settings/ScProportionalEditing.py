...
class ScProportionalEditing(Node, ScSettingNode):
    ...
    in_size: FloatProperty(
        default=1.0, min=0.00001, max=5000, update=ScNode.update_value
    )
    ...
    def error_condition(self):
        return (
            ...
            or (
                not self.inputs["Falloff"].default_value
                in [
                    "SMOOTH",
                    "SPHERE",
                    "ROOT",
            ...
            or (
                self.inputs["Size"].default_value < 0.00001
                or self.inputs["Size"].default_value > 5000
            )
        )

    def functionality(self):
        bpy.context.scene.tool_settings.proportional_edit_falloff = self.inputs[
            "Falloff"
        ].default_value
        bpy.context.scene.tool_settings.proportional_size = self.inputs[
            "Size"
        ].default_value
        bpy.context.scene.tool_settings.use_proportional_connected = self.inputs[
            "Connected Only"
        ].default_value
        bpy.context.scene.tool_settings.use_proportional_edit = self.inputs[
            "Edit Mode"
        ].default_value
        bpy.context.scene.tool_settings.use_proportional_edit_objects = self.inputs[
            "Object Mode"
        ].default_value
