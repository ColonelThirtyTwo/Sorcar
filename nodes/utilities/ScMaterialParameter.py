...
class ScMaterialParameter(Node, ScNode):
    ...
    prop_mat: PointerProperty(
        name="Material", type=bpy.types.Material, update=ScNode.update_value
    )
            ...
            if not self.prop_node == "":
                ...
                if self.prop_type == "INPUT":
                    layout.prop_search(
                        self,
                        "prop_socket",
                        self.prop_mat.node_tree.nodes[self.prop_node],
                        "inputs",
                    )
                else:
                    layout.prop_search(
    ...
    def functionality(self):
        if self.prop_type == "INPUT":
            self.prop_mat.node_tree.nodes[self.prop_node].inputs[
                self.prop_socket
            ].default_value = eval(self.inputs["Value"].default_value)
        else:
            self.prop_mat.node_tree.nodes[self.prop_node].outputs[
                self.prop_socket
            ].default_value = eval(self.inputs["Value"].default_value)
