...
class ScBooleanOp(Node, ScNode):
    ...
    def error_condition(self):
        return not self.inputs["Operation"].default_value in [
            "AND",
            "OR",
            "EQUAL",
            "NOTEQUAL",
    ...
    def post_execute(self):
        ...
        if self.inputs["Operation"].default_value == "AND":
            out["Value"] = (
                self.inputs["X"].default_value and self.inputs["Y"].default_value
            )
        elif self.inputs["Operation"].default_value == "OR":
            out["Value"] = (
                self.inputs["X"].default_value or self.inputs["Y"].default_value
            )
        elif self.inputs["Operation"].default_value == "EQUAL":
            out["Value"] = (
                self.inputs["X"].default_value == self.inputs["Y"].default_value
            )
