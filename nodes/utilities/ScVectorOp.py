...
class ScVectorOp(Node, ScNode):
    ...
    in_op: EnumProperty(
        ...
        items=[
            ("ADD", "X + Y", "Addition"),
            ("SUB", "X - Y", "Subtraction"),
            ("MULT", "K * X", "Multiplication by Scalar"),
            ("CROSS", "X * Y", "Cross Product"),
            ("DOT", "X . Y", "Dot Product"),
            ("ANGLE", "Angle", ""),
            ("PROJ", "Project", ""),
            ("REFL", "Reflect", "Mirror"),
            ("ROT", "Rotation Difference", "Rotation Difference"),
            ("NORM", "Normalise X", "Unit Vector"),
            ("ORTHO", "Orthogonal X", "Perpendicular"),
            ("LERP", "Lerp", "Linear Interpolate"),
            ("SLERP", "S-Lerp", "Spherical Interpolate"),
        ],
        default="ADD",
    ...
    def error_condition(self):
        return (
            ...
            self.inputs["Operation"].default_value == "LERP"
            and (
                self.inputs["K"].default_value < 0 or self.inputs["K"].default_value > 1
            )
        )
    ...
    def post_execute(self):
        ...
        if self.inputs["Operation"].default_value == "ADD":
            ...
            out["Value"] = (
                Vector(self.inputs["X"].default_value) * self.inputs["K"].default_value
            )
            ...
            out["Value"] = (
                Vector(self.inputs["X"].default_value)
                .rotation_difference(Vector(self.inputs["Y"].default_value))
                .to_euler()
            )
            ...
            out["Value"] = Vector(self.inputs["X"].default_value).lerp(
                Vector(self.inputs["Y"].default_value), self.inputs["K"].default_value
            )
