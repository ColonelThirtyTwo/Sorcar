...
class ScFlatten(Node, ScEditOperatorNode):
    ...
    in_mode: EnumProperty(
        items=[("FACES", "Faces", ""), ("VERTICES", "Vertices", "")],
        default="FACES",
        update=ScNode.update_value,
    )
    in_factor: FloatProperty(
        default=0.5, min=-10.0, max=10.0, update=ScNode.update_value
    )
    ...
    in_lambda_factor: FloatProperty(
        default=0.5, min=0.0000001, max=1000.0, update=ScNode.update_value
    )
    ...
    def error_condition(self):
        return (
            ...
            or (
                self.inputs["Factor"].default_value < -10.0
                or self.inputs["Factor"].default_value > 10.0
            )
            or (
                int(self.inputs["Repeat"].default_value) < 0
                or int(self.inputs["Repeat"].default_value) > 1000
            )
            or (
                self.inputs["Lambda Factor"].default_value < 0.0000001
                or self.inputs["Lambda Factor"].default_value > 1000.0
            )
            or (
                self.inputs["Lambda Border"].default_value < 0.0000001
                or self.inputs["Lambda Border"].default_value > 1000.0
            )
        )
