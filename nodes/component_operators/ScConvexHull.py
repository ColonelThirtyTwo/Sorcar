...
class ScConvexHull(Node, ScEditOperatorNode):
    ...
    in_face_threshold: FloatProperty(
        default=0.698132,
        min=0.0,
        max=3.14159,
        unit="ROTATION",
    ...
    in_shape_threshold: FloatProperty(
        default=0.698132,
        min=0.0,
        max=3.14159,
        unit="ROTATION",
    ...
    def init(self, context):
        ...
        self.inputs.new("ScNodeSocketBool", "Use Existing Faces").init(
            "in_use_existing_faces"
        )
        ...
        self.inputs.new("ScNodeSocketNumber", "Max Face Angle").init(
            "in_face_threshold", True
        )
        self.inputs.new("ScNodeSocketNumber", "Max Shape Angle").init(
            "in_shape_threshold"
        )
    ...
    def error_condition(self):
        return (
            ...
            or (
                self.inputs["Max Face Angle"].default_value < 0.0
                or self.inputs["Max Face Angle"].default_value > 3.14159
            )
            or (
                self.inputs["Max Shape Angle"].default_value < 0.0
                or self.inputs["Max Shape Angle"].default_value > 3.14159
            )
        )
