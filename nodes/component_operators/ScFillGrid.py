...
class ScFillGrid(Node, ScEditOperatorNode):
    ...
    def error_condition(self):
        return (
            ...
            or (
                int(self.inputs["Span"].default_value) < 1
                or int(self.inputs["Span"].default_value) > 1000
            )
            or (
                int(self.inputs["Offset"].default_value) < -1000
                or int(self.inputs["Offset"].default_value) > 1000
            )
        )
