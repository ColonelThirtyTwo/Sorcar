...
class ScWireframe(Node, ScEditOperatorNode):
    ...
    in_thickness: FloatProperty(
        default=0.01, min=0.0, max=10000.0, update=ScNode.update_value
    )
    in_offset: FloatProperty(
        default=0.01, min=0.0, max=10000.0, update=ScNode.update_value
    )
    in_use_crease: BoolProperty(update=ScNode.update_value)
    in_crease_weight: FloatProperty(
        default=0.01, min=0.0, max=1000.0, update=ScNode.update_value
    )
    ...
    def error_condition(self):
        return (
            ...
            or (
                self.inputs["Thickness"].default_value < 0.0
                or self.inputs["Thickness"].default_value > 10000.0
            )
            or (
                self.inputs["Offset"].default_value < 0.0
                or self.inputs["Offset"].default_value > 10000.0
            )
            or (
                self.inputs["Crease Weight"].default_value < 0.0
                or self.inputs["Crease Weight"].default_value > 1000.0
            )
        )
