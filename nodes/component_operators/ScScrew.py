...
class ScScrew(Node, ScEditOperatorNode):
    ...
    in_axis: FloatVectorProperty(
        default=(1.0, 0.0, 0.0), min=-1.0, max=1.0, update=ScNode.update_value
    )
    ...
    def error_condition(self):
        return (
            ...
            or (
                int(self.inputs["Steps"].default_value) < 1
                or int(self.inputs["Steps"].default_value) > 100000
            )
            or (
                int(self.inputs["Turns"].default_value) < 1
                or int(self.inputs["Turns"].default_value) > 100000
            )
