...
class ScPoke(Node, ScEditOperatorNode):
    ...
    def init(self, context):
        ...
        self.inputs.new("ScNodeSocketBool", "Relative Offset").init(
    ...
    def error_condition(self):
        return (
            ...
            or (
                self.inputs["Poke Offset"].default_value < -1000
                or self.inputs["Poke Offset"].default_value > 1000
            )
            or (
                not self.inputs["Poke Center"].default_value
                in ["MEDIAN_WEIGHTED", "MEDIAN", "BOUNDS"]
            )
        )
