...
class ScAverageNormals(Node, ScEditOperatorNode):
    ...
    in_weight: FloatProperty(
        default=50.0, min=1.0, max=100.0, update=ScNode.update_value
    )
    ...
    def error_condition(self):
        return (
            ...
            or (
                ...
                in ["CUSTOM_NORMAL", "FACE_AREA", "CORNER_ANGLE"]
            )
            or (
                self.inputs["Weight"].default_value < 1.0
                or self.inputs["Weight"].default_value > 100.0
            )
            or (
