...
class ScBridgeEdgeLoops(Node, ScEditOperatorNode):
    ...
    in_merge_factor: FloatProperty(
        default=0.5, min=0.0, max=1.0, update=ScNode.update_value
    )
    in_twist_offset: IntProperty(
        default=0, min=-1000, max=1000, update=ScNode.update_value
    )
    in_number_cuts: IntProperty(default=0, min=0, max=1000, update=ScNode.update_value)
    in_interpolation: EnumProperty(
        items=[
            ("LINEAR", "Linear", ""),
            ("PATH", "Path", ""),
            ("SURFACE", "Surface", ""),
    ...
    in_smoothness: FloatProperty(
        default=1.0, min=0.0, max=1000.0, update=ScNode.update_value
    )
    in_profile_shape_factor: FloatProperty(
        default=0.0, min=-1000.0, max=1000.0, update=ScNode.update_value
    )
    in_profile_shape: EnumProperty(
        items=[
            ("SMOOTH", "Smooth", ""),
            ("SPHERE", "Sphere", ""),
            ("ROOT", "Root", ""),
    ...
    def init(self, context):
        ...
        self.inputs.new("ScNodeSocketNumber", "Number of Cuts").init(
            "in_number_cuts", True
        )
        ...
        self.inputs.new("ScNodeSocketNumber", "Profile Factor").init(
            "in_profile_shape_factor"
        )
    ...
    def error_condition(self):
        return (
            ...
            or (
                not self.inputs["Connect Loops"].default_value
                in ["SINGLE", "CLOSED", "PAIRS"]
            )
            or (
                self.inputs["Merge Factor"].default_value < 0.0
                or self.inputs["Merge Factor"].default_value > 1.0
            )
            or (
                int(self.inputs["Twist"].default_value) < -1000
                or int(self.inputs["Twist"].default_value) > 1000
            )
            or (
                int(self.inputs["Number of Cuts"].default_value) < 0
                or int(self.inputs["Number of Cuts"].default_value) > 1000
            )
            or (
                not self.inputs["Interpolation"].default_value
                in ["LINEAR", "PATH", "SURFACE"]
            )
            or (
                self.inputs["Smoothness"].default_value < 0.0
                or self.inputs["Smoothness"].default_value > 1000.0
            )
            or (
                self.inputs["Profile Factor"].default_value < -1000.0
                or self.inputs["Smoothness"].default_value > 1000.0
            )
            or (
                not self.inputs["Profile Shape"].default_value
                in ["SMOOTH", "SPHERE", "ROOT", "INVERSE_SQUARE", "SHARP", "LINEAR"]
            )
        )
