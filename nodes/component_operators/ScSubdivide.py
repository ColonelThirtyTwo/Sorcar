...
class ScSubdivide(Node, ScEditOperatorNode):
    ...
    in_quadcorner: EnumProperty(
        items=[
            ("INNERVERT", "Inner Vertices", ""),
            ("PATH", "Path", ""),
    ...
    in_fractal_along_normal: FloatProperty(
    ...
    def init(self, context):
        ...
        self.inputs.new("ScNodeSocketNumber", "Along Normal").init(
            "in_fractal_along_normal"
        )
    ...
    def error_condition(self):
        return (
            ...
            or (
                int(self.inputs["Number of Cuts"].default_value) < 1
                or int(self.inputs["Number of Cuts"].default_value) > 100
            )
            or (
            ...
            or (
                not self.inputs["Quad Corner Type"].default_value
                in ["INNERVERT", "PATH", "STRAIGHT_CUT", "FAN"]
            )
            or (
                self.inputs["Fractal"].default_value < 0.0
                or self.inputs["Fractal"].default_value > 1000000.0
            )
            or (
                self.inputs["Along Normal"].default_value < 0.0
                or self.inputs["Along Normal"].default_value > 1.0
            )
