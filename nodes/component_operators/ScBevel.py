...
class ScBevel(Node, ScEditOperatorNode):
    ...
    in_profile: FloatProperty(
        default=0.5, min=0.15, max=1.0, update=ScNode.update_value
    )
    ...
    in_miter_outer: EnumProperty(
        items=[("SHARP", "Sharp", ""), ("PATCH", "Patch", ""), ("ARC", "Arc", "")],
        default="SHARP",
        update=ScNode.update_value,
    )
    in_miter_inner: EnumProperty(
        items=[("SHARP", "Sharp", ""), ("ARC", "Arc", "")],
        default="SHARP",
        update=ScNode.update_value,
    )
    in_spread: FloatProperty(
        default=0.1, min=0.0, max=1000000.0, update=ScNode.update_value
    )
    ...
    def error_condition(self):
        return (
            ...
            or (
                not self.inputs["Offset Type"].default_value
                in ["OFFSET", "WIDTH", "DEPTH", "PERCENT"]
            )
            or (
                self.inputs["Offset Type"].default_value == "PERCENT"
                and (
                    self.inputs["Offset"].default_value < 0.0
                    or self.inputs["Offset"].default_value > 100.0
                )
            )
            or (
                self.inputs["Offset"].default_value < 0.0
                or self.inputs["Offset"].default_value > 1000000.0
            )
            or (
                int(self.inputs["Segments"].default_value) < 1
                or int(self.inputs["Segments"].default_value) > 1000
            )
            or (
                self.inputs["Profile"].default_value < 0.15
            ...
            or (
                self.inputs["Spread"].default_value < 0.0
                or self.inputs["Spread"].default_value > 1000000.0
            )
        )
