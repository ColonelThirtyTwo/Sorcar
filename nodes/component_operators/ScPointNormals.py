...
class ScPointNormals(Node, ScEditOperatorNode):
    ...
    in_spherize_strength: FloatProperty(
        default=0.1, min=0.0, max=1.0, update=ScNode.update_value
    )
    ...
    def init(self, context):
        ...
        self.inputs.new("ScNodeSocketNumber", "Spherize Strength").init(
            "in_spherize_strength"
        )

    def error_condition(self):
        return super().error_condition() or (
            self.inputs["Spherize Strength"].default_value < 0.0
            or self.inputs["Spherize Strength"].default_value > 1.0
        )
