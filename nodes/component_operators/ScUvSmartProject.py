...
class ScUvSmartProject(Node, ScEditOperatorNode):
    ...
    def init(self, context):
        ...
        self.inputs.new("ScNodeSocketBool", "Stretch to UV Bounds").init(
            "in_stretch", True
        )
    ...
    def error_condition(self):
        return (
            ...
            or (
                self.inputs["Angle Limit"].default_value < 1.0
                or self.inputs["Angle Limit"].default_value > 89.0
            )
            or (
                self.inputs["Island Margin"].default_value < 0.0
                or self.inputs["Island Margin"].default_value > 1.0
            )
            or (
                self.inputs["Area Weight"].default_value < 0.0
                or self.inputs["Area Weight"].default_value > 1.0
            )
        )
