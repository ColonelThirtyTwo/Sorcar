...
class ScAddCylinder(Node, ScEditOperatorNode):
    bl_idname = "ScAddCylinder"
    ...
    in_vertices: IntProperty(
        default=32, min=3, max=10000000, update=ScNode.update_value
    )
    ...
    def error_condition(self):
        return (
            ...
            or (
                not self.inputs["Cap Fill Type"].default_value
                in ["NOTHING", "NGON", "TRIFAN"]
            )
            or (
                self.inputs["Vertices"].default_value < 3
                or self.inputs["Vertices"].default_value > 10000000
            )
