...
class ScRandomizeTransform(Node, ScObjectOperatorNode):
    ...
    in_rot: FloatVectorProperty(
        min=-3.14159, max=3.14159, unit="ROTATION", update=ScNode.update_value
    )
    in_use_scale: BoolProperty(default=True, update=ScNode.update_value)
    in_scale_even: BoolProperty(update=ScNode.update_value)
    in_scale: FloatVectorProperty(
        default=(1.0, 1.0, 1.0), min=-100.0, max=100.0, update=ScNode.update_value
    )
    ...
    def init(self, context):
        ...
        self.inputs.new("ScNodeSocketBool", "Randomize Location").init(
            "in_use_loc", True
        )
    ...
    def error_condition(self):
        return (
            ...
            or (
                Vector(self.inputs["Location"].default_value).magnitude < -100.0
                or Vector(self.inputs["Location"].default_value).magnitude > 100.0
            )
            or (
                Vector(self.inputs["Rotation"].default_value).magnitude < -3.14159
                or Vector(self.inputs["Rotation"].default_value).magnitude > 3.14159
            )
            or (
                Vector(self.inputs["Scale"].default_value).magnitude < -100.0
                or Vector(self.inputs["Scale"].default_value).magnitude > 100.0
            )
        )
