...
class ScSelectByMaterial(Node, ScSelectionNode):
    ...
    def error_condition(self):
        return (
            ...
            or self.inputs["Object"].default_value.material_slots.find(
                self.prop_mat.name
            )
            == -1
        )
    ...
    def functionality(self):
        self.inputs["Object"].default_value.active_material_index = self.inputs[
            "Object"
        ].default_value.material_slots.find(self.prop_mat.name)
        if self.inputs["Deselect"].default_value:
