...
class ScSelectAlternateFaces(Node, ScSelectionNode):
    ...
    def functionality(self):
        ...
        for i in range(
            int(self.inputs["Offset"].default_value),
            len(self.inputs["Object"].default_value.data.polygons),
            int(self.inputs["Every Nth"].default_value),
        ):
            self.inputs["Object"].default_value.data.polygons[
                i
            ].select = not self.inputs["Deselect"].default_value
