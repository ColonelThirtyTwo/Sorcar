...
class ScBeginForEachComponentLoop(Node, ScNode):
    ...
    in_type: EnumProperty(
        items=[("VERT", "Vertices", ""), ("EDGE", "Edges", ""), ("FACE", "Faces", "")],
        default="FACE",
    ...
    def pre_execute(self):
        ...
        if self.inputs["Type"].default_value == "VERT":
            self.prop_components = repr(
                [
                    i.index
                    for i in self.inputs["In"].default_value.data.vertices
                    if i.select
                ]
            )
            ...
            self.prop_components = repr(
                [
                    i.index
                    for i in self.inputs["In"].default_value.data.edges
                    if i.select
                ]
            )
            ...
            self.prop_components = repr(
                [
                    i.index
                    for i in self.inputs["In"].default_value.data.polygons
                    if i.select
                ]
            )
