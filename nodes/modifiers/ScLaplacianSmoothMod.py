...
class ScLaplacianSmoothMod(Node, ScModifierNode):
    ...
    in_lambda_factor: FloatProperty(
        default=0.01, soft_min=-1000.0, soft_max=1000.0, update=ScNode.update_value
    )
    in_lambda_border: FloatProperty(
        default=0.01, soft_min=-1000.0, soft_max=1000.0, update=ScNode.update_value
    )
    in_use_volume_preserve: BoolProperty(
        name="Preserve Volume", default=True, update=ScNode.update_value
    )
    in_use_normalized: BoolProperty(
        name="Normalized", default=True, update=ScNode.update_value
    )
    ...
    def init(self, context):
        ...
        self.inputs.new("ScNodeSocketNumber", "Preserve Volume").init(
            "in_use_volume_preserve"
        )
    ...
    def error_condition(self):
        return super().error_condition() or (
            self.inputs["Repeat"].default_value < -32768
            or self.inputs["Repeat"].default_value > 32767
        )
    ...
    def functionality(self):
        ...
        bpy.context.object.modifiers[self.prop_mod_name].iterations = self.inputs[
            "Repeat"
        ].default_value
        ...
        bpy.context.object.modifiers[self.prop_mod_name].use_z = self.inputs[
            "Z"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].lambda_factor = self.inputs[
            "Factor"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].lambda_border = self.inputs[
            "Border"
        ].default_value
        bpy.context.object.modifiers[
            self.prop_mod_name
        ].use_volume_preserve = self.inputs["Preserve Volume"].default_value
        bpy.context.object.modifiers[self.prop_mod_name].use_normalized = self.inputs[
            "Normalized"
        ].default_value
