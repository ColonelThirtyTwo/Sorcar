...
class ScArrayMod(Node, ScModifierNode):
    ...
    in_curve: PointerProperty(
        type=bpy.types.Object, poll=sc_poll_curve, update=ScNode.update_value
    )
    ...
    in_merge_threshold: FloatProperty(
        default=0.01, min=0.0, max=1.0, update=ScNode.update_value
    )
    ...
    in_start_cap: PointerProperty(
        type=bpy.types.Object, poll=sc_poll_mesh, update=ScNode.update_value
    )
    in_end_cap: PointerProperty(
        type=bpy.types.Object, poll=sc_poll_mesh, update=ScNode.update_value
    )
    ...
    def init(self, context):
        ...
        self.inputs.new("ScNodeSocketBool", "Use Constant Offset").init(
            "in_use_constant_offset"
        )
        self.inputs.new("ScNodeSocketVector", "Constant Offset").init(
            "in_constant_offset_displace"
        )
        self.inputs.new("ScNodeSocketBool", "Use Relative Offset").init(
            "in_use_relative_offset"
        )
        self.inputs.new("ScNodeSocketVector", "Relative Offset").init(
            "in_relative_offset_displace", True
        )
        self.inputs.new("ScNodeSocketBool", "Use Object Offset").init(
            "in_use_object_offset"
        )
        ...
        self.inputs.new("ScNodeSocketBool", "First Last").init(
            "in_use_merge_vertices_cap"
        )
    ...
    def error_condition(self):
        return (
            ...
            or (
                not self.inputs["Fit Type"].default_value
                in ["FIXED_COUNT", "FIT_LENGTH", "FIT_CURVE"]
            )
            or (
                int(self.inputs["Count"].default_value) < 1
            ...
            or (
                self.inputs["U Offset"].default_value < -1.0
                or self.inputs["U Offset"].default_value > 1.0
            )
            or (
                self.inputs["V Offset"].default_value < -1.0
                or self.inputs["V Offset"].default_value > 1.0
            )
        )

    def functionality(self):
        bpy.context.object.modifiers[self.prop_mod_name].fit_type = self.inputs[
            "Fit Type"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].count = int(
            self.inputs["Count"].default_value
        )
        bpy.context.object.modifiers[self.prop_mod_name].fit_length = self.inputs[
            "Length"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].curve = self.inputs[
            "Curve"
        ].default_value
        bpy.context.object.modifiers[
            self.prop_mod_name
        ].use_constant_offset = self.inputs["Use Constant Offset"].default_value
        bpy.context.object.modifiers[
            self.prop_mod_name
        ].constant_offset_displace = self.inputs["Constant Offset"].default_value
        bpy.context.object.modifiers[
            self.prop_mod_name
        ].use_merge_vertices = self.inputs["Merge"].default_value
        bpy.context.object.modifiers[
            self.prop_mod_name
        ].use_merge_vertices_cap = self.inputs["First Last"].default_value
        bpy.context.object.modifiers[self.prop_mod_name].merge_threshold = self.inputs[
            "Distance"
        ].default_value
        bpy.context.object.modifiers[
            self.prop_mod_name
        ].use_relative_offset = self.inputs["Use Relative Offset"].default_value
        bpy.context.object.modifiers[
            self.prop_mod_name
        ].relative_offset_displace = self.inputs["Relative Offset"].default_value
        bpy.context.object.modifiers[
            self.prop_mod_name
        ].use_object_offset = self.inputs["Use Object Offset"].default_value
        bpy.context.object.modifiers[self.prop_mod_name].offset_object = self.inputs[
            "Object Offset"
        ].default_value
        ...
        bpy.context.object.modifiers[self.prop_mod_name].start_cap = self.inputs[
            "End Cap"
        ].default_value
