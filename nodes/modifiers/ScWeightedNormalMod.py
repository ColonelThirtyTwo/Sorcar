...
class ScWeightedNormalMod(Node, ScModifierNode):
    ...
    in_mode: EnumProperty(
        ...
        items=[
            ...
            ("FACE_AREA_WITH_ANGLE", "Face Area and Angle", ""),
        ],
    ...
    in_threshold: FloatProperty(
        default=0.01, min=0.0, max=10.0, update=ScNode.update_value
    )
    ...
    def error_condition(self):
        return (
            ...
            or (
                not self.inputs["Weighting Mode"].default_value
                in ["FACE_AREA", "CORNER_ANGLE", "FACE_AREA_WITH_ANGLE"]
            )
            or (
                int(self.inputs["Weight"].default_value) < 1
                or int(self.inputs["Weight"].default_value) > 100
            )
            or (
                self.inputs["Threshold"].default_value < 0.0
                or self.inputs["Threshold"].default_value > 10.0
            )
        )
    ...
    def functionality(self):
        ...
        bpy.context.object.modifiers[self.prop_mod_name].weight = int(
            self.inputs["Weight"].default_value
        )
        bpy.context.object.modifiers[self.prop_mod_name].thresh = self.inputs[
            "Threshold"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].keep_sharp = self.inputs[
            "Keep Sharp"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].face_influence = self.inputs[
            "Face Influence"
        ].default_value
