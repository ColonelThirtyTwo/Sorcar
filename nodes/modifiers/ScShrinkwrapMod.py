...
class ScShrinkwrapMod(Node, ScModifierNode):
    ...
    in_cull: EnumProperty(
        name="Cull Faces",
        items=[("OFF", "Off", ""), ("FRONT", "Front", ""), ("BACK", "Back", "")],
        update=ScNode.update_value,
    )
    in_aux_target: PointerProperty(
        type=bpy.types.Object, poll=sc_poll_mesh, update=ScNode.update_value
    )
    ...
    def error_condition(self):
        return (
            ...
            or (
                not self.inputs["Mode"].default_value
                in [
            ...
            or (
                not self.inputs["Snap Mode"].default_value
                in [
                    "ON_SURFACE",
                    "INSIDE",
                    "OUTSIDE",
            ...
            or (
                int(self.inputs["Subsurf Levels"].default_value) < 0
                or int(self.inputs["Subsurf Levels"].default_value) > 6
            )
    ...
    def functionality(self):
        ...
        bpy.context.object.modifiers[self.prop_mod_name].target = self.inputs[
            "Target"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].offset = self.inputs[
            "Offset"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].wrap_method = self.inputs[
            "Mode"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].wrap_mode = self.inputs[
            "Snap Mode"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].subsurf_levels = int(
            self.inputs["Subsurf Levels"].default_value
        )
        bpy.context.object.modifiers[self.prop_mod_name].project_limit = self.inputs[
            "Limit"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].use_project_x = self.inputs[
            "X"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].use_project_y = self.inputs[
            "Y"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].use_project_z = self.inputs[
            "Z"
        ].default_value
        bpy.context.object.modifiers[
            self.prop_mod_name
        ].use_negative_direction = self.inputs["Negative"].default_value
        bpy.context.object.modifiers[
            self.prop_mod_name
        ].use_positive_direction = self.inputs["Positive"].default_value
        bpy.context.object.modifiers[self.prop_mod_name].use_invert_cull = self.inputs[
            "Invert Cull"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].cull_face = self.inputs[
            "Cull Faces"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].auxiliary_target = self.inputs[
            "Auxiliary Target"
        ].default_value
