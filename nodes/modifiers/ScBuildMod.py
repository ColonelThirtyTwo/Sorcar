...
class ScBuildMod(Node, ScModifierNode):
    ...
    in_percent: FloatProperty(
        default=50.0, min=0.0, max=100.0, update=ScNode.update_value
    )
    ...
    def error_condition(self):
        return (
            ...
            or (
                self.inputs["Percent"].default_value < 0.0
                or self.inputs["Percent"].default_value > 100.0
            )
            or (
                int(self.inputs["Seed"].default_value) < 1
                or int(self.inputs["Seed"].default_value > 1048574)
            )
        )

    def functionality(self):
        bpy.context.object.modifiers[self.prop_mod_name].frame_start = (
            bpy.context.scene.frame_current_final - self.inputs["Percent"].default_value
        )
        bpy.context.object.modifiers[self.prop_mod_name].frame_duration = 100.0
        bpy.context.object.modifiers[self.prop_mod_name].use_reverse = self.inputs[
            "Reverse"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].use_random_order = self.inputs[
            "Randomize"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].seed = int(
            self.inputs["Seed"].default_value
        )
