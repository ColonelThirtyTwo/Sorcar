...
class ScCorrectiveSmoothnessMod(Node, ScModifierNode):
    ...
    in_smooth_type: EnumProperty(
        items=[("SIMPLE", "Simple", ""), ("LENGTH_WEIGHTED", "Length Weight", "")],
        default="SIMPLE",
        update=ScNode.update_value,
    )
    ...
    def init(self, context):
        ...
        self.inputs.new("ScNodeSocketBool", "Pin Boundaries").init(
            "in_use_pin_boundary"
        )
    ...
    def error_condition(self):
        return (
            ...
            or (
                int(self.inputs["Repeat"].default_value) < -32768
                or int(self.inputs["Repeat"].default_value) > 32767
            )
            or (
                not self.inputs["Smooth Type"].default_value
                in ["SIMPLE", "LENGTH_WEIGHTED"]
            )
        )
    ...
    def functionality(self):
        ...
        bpy.context.object.modifiers[self.prop_mod_name].iterations = int(
            self.inputs["Repeat"].default_value
        )
        bpy.context.object.modifiers[self.prop_mod_name].smooth_type = self.inputs[
            "Smooth Type"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].use_only_smooth = self.inputs[
            "Only Smooth"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].use_pin_boundary = self.inputs[
            "Pin Boundaries"
        ].default_value
