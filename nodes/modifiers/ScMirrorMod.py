...
class ScMirrorMod(Node, ScModifierNode):
    ...
    in_merge_threshold: FloatProperty(
        default=0.001, min=0, soft_max=1, update=ScNode.update_value
    )
    in_use_mirror_u: BoolProperty(update=ScNode.update_value)
    in_use_mirror_v: BoolProperty(update=ScNode.update_value)
    in_mirror_offset_u: FloatProperty(
        default=0.0, min=-1.0, max=1.0, update=ScNode.update_value
    )
    in_mirror_offset_v: FloatProperty(
        default=0.0, min=-1.0, max=1.0, update=ScNode.update_value
    )
    in_offset_u: FloatProperty(
        default=0.0,
        soft_min=-1,
        soft_max=1,
        min=-1000,
    ...
    def init(self, context):
        ...
        self.inputs.new("ScNodeSocketBool", "Vertex Groups").init(
            "in_use_mirror_vertex_groups"
        )
        ...
        self.inputs.new("ScNodeSocketNumber", "Flip U Offset").init(
            "in_mirror_offset_u"
        )
        self.inputs.new("ScNodeSocketNumber", "Flip V Offset").init(
            "in_mirror_offset_v"
        )
    ...
    def error_condition(self):
        return (
            ...
            or (
                self.inputs["Flip U"].default_value < -1
                or self.inputs["Flip U"].default_value > 1
            )
            or (
                self.inputs["Flip V"].default_value < -1
                or self.inputs["Flip V"].default_value > 1
            )
            or (
                self.inputs["U Offset"].default_value < -1000
                or self.inputs["U Offset"].default_value > 1000
            )
            or (
                self.inputs["V Offset"].default_value < -1000
                or self.inputs["V Offset"].default_value > 1000
            )
        )

    def functionality(self):
        bpy.context.object.modifiers[self.prop_mod_name].use_axis[0] = self.inputs[
            "Axis X"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].use_axis[1] = self.inputs[
            "Axis Y"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].use_axis[2] = self.inputs[
            "Axis Z"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].use_bisect_axis[
            0
        ] = self.inputs["Bisect X"].default_value
        bpy.context.object.modifiers[self.prop_mod_name].use_bisect_axis[
            1
        ] = self.inputs["Bisect Y"].default_value
        bpy.context.object.modifiers[self.prop_mod_name].use_bisect_axis[
            2
        ] = self.inputs["Bisect Z"].default_value
        bpy.context.object.modifiers[self.prop_mod_name].use_bisect_flip_axis[
            0
        ] = self.inputs["Flip X"].default_value
        bpy.context.object.modifiers[self.prop_mod_name].use_bisect_flip_axis[
            1
        ] = self.inputs["Flip Y"].default_value
        bpy.context.object.modifiers[self.prop_mod_name].use_bisect_flip_axis[
            2
        ] = self.inputs["Flip Z"].default_value
        bpy.context.object.modifiers[self.prop_mod_name].mirror_object = self.inputs[
            "Mirror Object"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].use_mirror_merge = self.inputs[
            "Merge"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].use_clip = self.inputs[
            "Clipping"
        ].default_value
        bpy.context.object.modifiers[
            self.prop_mod_name
        ].use_mirror_vertex_groups = self.inputs["Vertex Groups"].default_value
        bpy.context.object.modifiers[self.prop_mod_name].merge_threshold = self.inputs[
            "Merge Limit"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].use_mirror_u = self.inputs[
            "Flip U"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].use_mirror_v = self.inputs[
            "Flip V"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].mirror_offset_u = self.inputs[
            "Flip U Offset"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].mirror_offset_v = self.inputs[
            "Flip V Offset"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].offset_u = self.inputs[
            "U Offset"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].offset_v = self.inputs[
            "V Offset"
        ].default_value
