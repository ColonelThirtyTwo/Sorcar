...
class ScDisplaceMod(Node, ScModifierNode):
    ...
    in_space: EnumProperty(
        items=[("LOCAL", "Local", ""), ("GLOBAL", "Global", "")],
        default="LOCAL",
        update=ScNode.update_value,
    )
    in_texture_coords: EnumProperty(
        items=[
            ("LOCAL", "Local", ""),
            ("GLOBAL", "Global", ""),
            ("OBJECT", "Object", ""),
    ...
    in_texture_coords_object: PointerProperty(
        type=bpy.types.Object, update=ScNode.update_value
    )
    in_mid_level: FloatProperty(
        default=0.5, soft_min=0.0, soft_max=1.0, update=ScNode.update_value
    )
    in_strength: FloatProperty(
        default=1.0, soft_min=-100.0, soft_max=100.0, update=ScNode.update_value
    )
    ...
    def init(self, context):
        ...
        self.inputs.new("ScNodeSocketString", "Texture Coordinates").init(
            "in_texture_coords"
        )
        self.inputs.new("ScNodeSocketObject", "Secondary Object").init(
            "in_texture_coords_object"
        )
    ...
    def draw_buttons(self, context, layout):
        ...
        if not self.inputs["Object"].default_value == None:
            ...
            if self.inputs["Texture Coordinates"].default_value == "UV":
                layout.prop_search(
                    self, "prop_uv_layer", self.mesh.data, "uv_textures", text=""
                )
    ...
    def error_condition(self):
        return (
            ...
            or (
                not self.inputs["Direction"].default_value
                in ["X", "Y", "Z", "NORMAL", "CUSTOM_NORMAL", "RGB_TO_XYZ"]
            )
            or (not self.inputs["Space"].default_value in ["LOCAL", "GLOBAL"])
            or (
                not self.inputs["Texture Coordinates"].default_value
                in ["LOCAL", "GLOBAL", "OBJECT", "UV"]
            )
        )
    ...
    def functionality(self):
        ...
        bpy.context.object.modifiers[self.prop_mod_name].direction = self.inputs[
            "Direction"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].space = self.inputs[
            "Space"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].texture_coords = self.inputs[
            "Texture Coordinates"
        ].default_value
        bpy.context.object.modifiers[
            self.prop_mod_name
        ].texture_coords_object = self.inputs["Secondary Object"].default_value
        bpy.context.object.modifiers[self.prop_mod_name].mid_level = self.inputs[
            "Midlevel"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].strength = self.inputs[
            "Strength"
        ].default_value
