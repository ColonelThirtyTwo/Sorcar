...
class ScCastMod(Node, ScModifierNode):
    ...
    in_control_object: PointerProperty(
    ...
    def init(self, context):
        ...
        self.inputs.new("ScNodeSocketObject", "Control Object").init(
            "in_control_object"
        )
    ...
    def error_condition(self):
        return (
            ...
            or (
                not self.inputs["Cast Type"].default_value
                in ["SPHERE", "CYLINDER", "CUBOID"]
            )
    ...
    def functionality(self):
        bpy.context.object.modifiers[self.prop_mod_name].cast_type = self.inputs[
            "Cast Type"
        ].default_value
        ...
        bpy.context.object.modifiers[self.prop_mod_name].radius = self.inputs[
            "Radius"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].size = self.inputs[
            "Size"
        ].default_value
        bpy.context.object.modifiers[
            self.prop_mod_name
        ].use_radius_as_size = self.inputs["From Radius"].default_value
        bpy.context.object.modifiers[
            self.prop_mod_name
        ...
        bpy.context.object.modifiers[self.prop_mod_name].use_transform = self.inputs[
            "Use Transform"
        ].default_value
