...
class ScEdgeSplitMod(Node, ScModifierNode):
    ...
    def init(self, context):
        ...
        self.inputs.new("ScNodeSocketNumber", "Split Angle").init(
            "in_split_angle", True
        )
    ...
    def error_condition(self):
        return super().error_condition() or (
            self.inputs["Split Angle"].default_value < 0
            or self.inputs["Split Angle"].default_value > 3.14159
        )

    def functionality(self):
        bpy.context.object.modifiers[self.prop_mod_name].split_angle = self.inputs[
            "Split Angle"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].use_edge_angle = self.inputs[
            "Edge Angle"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].use_edge_sharp = self.inputs[
            "Sharp Edges"
        ].default_value
