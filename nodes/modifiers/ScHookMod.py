...
class ScHookMod(Node, ScModifierNode):
    ...
    def init(self, context):
        ...
        self.inputs.new("ScNodeSocketBool", "Uniform Faloff").init(
            "in_use_falloff_uniform"
        )
    ...
    def error_condition(self):
        return (
            ...
            or (
                self.inputs["Strength"].default_value < 0
                or self.inputs["Strength"].default_value > 1
            )
            or (
                not self.inputs["Falloff Type"].default_value
                in [
                    "NONE",
                    "SMOOTH",
                    "SPHERE",
    ...
    def functionality(self):
        ...
        bpy.context.object.modifiers[self.prop_mod_name].falloff_radius = self.inputs[
            "Radius"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].strength = self.inputs[
            "Strength"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].falloff_type = self.inputs[
            "Falloff Type"
        ].default_value
        bpy.context.object.modifiers[
            self.prop_mod_name
        ].use_falloff_uniform = self.inputs["Uniform Faloff"].default_value
        bpy.context.object.modifiers[
            self.prop_mod_name
