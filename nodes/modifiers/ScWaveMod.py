...
class ScWaveMod(Node, ScModifierNode):
    ...
    in_speed: FloatProperty(
        default=0.25, soft_min=0.0, soft_max=1.0, update=ScNode.update_value
    )
    in_height: FloatProperty(
        default=0.5, soft_min=-2.0, soft_max=2.0, update=ScNode.update_value
    )
    in_width: FloatProperty(
        default=1.5, min=0.0, soft_max=5.0, update=ScNode.update_value
    )
    in_narrowness: FloatProperty(
        default=1.5, min=0.0, soft_max=10.0, update=ScNode.update_value
    )
    ...
    def error_condition(self):
        return (
            ...
            or (
                self.inputs["Offset"].default_value < 0.0
                or self.inputs["Offset"].default_value > 1048570.0
            )
    ...
    def functionality(self):
        bpy.context.object.modifiers[self.prop_mod_name].time_offset = (
            bpy.context.scene.frame_current - self.inputs["Offset"].default_value
        )
        bpy.context.object.modifiers[self.prop_mod_name].speed = self.inputs[
            "Speed"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].height = self.inputs[
            "Height"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].width = self.inputs[
            "Width"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].narrowness = self.inputs[
            "Narrowness"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].use_x = self.inputs[
            "Motion X"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].use_y = self.inputs[
            "Motion Y"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].use_cyclic = self.inputs[
            "Motion Cyclic"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].use_normal = self.inputs[
            "Normals"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].use_normal_x = self.inputs[
            "Normals X"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].use_normal_y = self.inputs[
            "Normals Y"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].use_normal_z = self.inputs[
            "Normals Z"
        ].default_value
