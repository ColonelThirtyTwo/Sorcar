...
class ScSkinMod(Node, ScModifierNode):
    ...
    in_branch_smoothing: FloatProperty(
    ...
    def init(self, context):
        ...
        self.inputs.new("ScNodeSocketNumber", "Branch Smoothing").init(
            "in_branch_smoothing", True
        )
        self.inputs.new("ScNodeSocketVector", "Skin Resize").init("in_resize", True)
        self.inputs.new("ScNodeSocketNumber", "Smooth Shading").init(
            "in_use_smooth_shade"
        )
    ...
    def error_condition(self):
        return super().error_condition() or (
            self.inputs["Branch Smoothing"].default_value < 0
            or self.inputs["Branch Smoothing"].default_value > 1
        )

    def functionality(self):
        bpy.context.object.modifiers[self.prop_mod_name].branch_smoothing = self.inputs[
            "Branch Smoothing"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].use_smooth_shade = self.inputs[
            "Smooth Shading"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].use_x_symmetry = self.inputs[
            "X"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].use_y_symmetry = self.inputs[
            "Y"
        ].default_value
        bpy.context.object.modifiers[self.prop_mod_name].use_z_symmetry = self.inputs[
            "Z"
        ].default_value
