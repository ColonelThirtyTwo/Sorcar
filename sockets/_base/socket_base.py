...
class ScNodeSocket:
        ...
        if self.is_output:
            ...
            if self.is_linked:
                ...
                if self.default_prop == "":
                    if (
                        self.node.bl_idname == "ScNodeGroup"
                        and [
                            i
                            for i in node.node_tree.inputs
                            if i.identifier == self.identifier
                        ][0].show_prop
                    ):
                        self.draw_layout(
                            context, layout, self, "default_value_update", text
                        )
                    else:
    ...
    def execute(self, forced):
        ...
        if self.is_output:
            ...
            if (
                self.is_linked
            ):  # self.is_linked doesn't get updated quickly (when using realtime update & modify links)
                ...
                if self.default_prop == "":
                    if (
                        ...
                        and [
                            i
                            for i in self.node.node_tree.inputs
                            if i.identifier == self.identifier
                        ][0].show_prop
                    ):
                        self.socket_error = False
                ...
                return self.set(
                    getattr(
                        bpy.data.node_groups[self.id_data.name].nodes[self.node.name],
                        self.default_prop,
                    )
                )
            return False
