    ...
    def execute(self, context):
        ...
        for link in links_external_in:
            ...
            node_group.links.new(
                group_input.outputs[""],
                node_group.nodes[link.to_node.name].inputs[link.to_socket.name],
            )
        ...
        for link in links_external_out:
            ...
            node_group.links.new(
                node_group.nodes[link.from_node.name].outputs[link.from_socket.name],
                group_output.inputs[""],
            )
        ...
        for i in range(0, len(links_external_in)):
            ...
            node_tree.links.new(
                link.from_node.outputs[link.from_socket.name], group_node.inputs[i]
            )
        for i in range(0, len(links_external_out)):
            link = links_external_out[i]
            node_tree.links.new(
                group_node.outputs[i], link.to_node.inputs[link.to_socket.name]
            )
